...
class Scrublet():
    def __init__(self, counts_matrix, total_counts=None, real_doublets=None, sim_doublet_ratio=2.0, n_neighbors=None, expected_doublet_rate=0.1, stdev_doublet_rate=0.02, random_state=0):
        ...
        if real_doublets is None:
            self._use_real_doublets = False
            self._doublet_count_matrix = None
        else:
            self._doublet_count_matrix = real_doublets
            self._use_real_doublets = True
    ...
    def scrub_doublets(self, synthetic_doublet_umi_subsampling=1.0, use_approx_neighbors=True, distance_metric='euclidean', get_doublet_neighbor_parents=False, min_counts=3, min_cells=3, min_gene_variability_pctl=85, log_transform=False, mean_center=True, normalize_variance=True, n_prin_comps=30, svd_solver='arpack', verbose=True):
        ...
        if self._use_real_doublets:
            print_optional('Using real doublets...', verbose)
            self.use_real_doublets(doublet_count_matrix = self._doublet_count_matrix)
    ...
    def use_real_doublets(self, doublet_count_matrix):

        self._E_sim = doublet_count_matrix
        self._total_counts_sim = self._E_sim.sum(1).A.squeeze()
        self.doublet_parents_ = None
